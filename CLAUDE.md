...
- **Default location**: Always use **Singapore** for test user location during IPCF. Do NOT use Sydney, California, or any other city. Singapore is the standard default for this project.
- **Cascading failure grouping**: When a setup step fails and blocks N subsequent checks, report them as a single grouped entry: `"INCONCLUSIVE — N checks blocked by <failure-id>"`. Do NOT list them as N separate inconclusive entries. Only expand to individual entries if the blocks have different root causes.
